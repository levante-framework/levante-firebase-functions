...
const firebaseBin = process.env.FIREBASE_BIN || path.resolve(__dirname, '..', 'node_modules', '.bin', 'firebase');
const firebaseConfigPath = process.env.FIREBASE_CONFIG_PATH || path.resolve(__dirname, '..', 'firebase.json');

const projectId = process.env.E2E_FIREBASE_PROJECT_ID || process.env.FIREBASE_PROJECT_ID || 'demo-emulator';
const cypressCacheFolder =
  process.env.CYPRESS_CACHE_FOLDER && !process.env.CYPRESS_CACHE_FOLDER.includes('/tmp/cursor-sandbox-cache')
...
const env = {
  ...
  E2E_VIDEO: 'false',
  E2E_SITE_NAME: process.env.E2E_SITE_NAME || '',
  E2E_AI_SUPER_ADMIN_EMAIL: process.env.E2E_AI_SUPER_ADMIN_EMAIL || 'superadmin@levante.test',
  E2E_AI_SUPER_ADMIN_PASSWORD: process.env.E2E_AI_SUPER_ADMIN_PASSWORD || 'super123',
  ...
  E2E_APP_DIR: appDir,
  VITE_BIN: process.env.VITE_BIN || path.resolve(appDir, 'node_modules', '.bin', 'vite'),
  E2E_EMULATOR_BOOTSTRAP_CMD:
    process.env.E2E_EMULATOR_BOOTSTRAP_CMD || `node ${JSON.stringify(bootstrapScript)}`,
};

console.log('=== STARTING CYPRESS UI EMULATOR SEED ===');
...
const result = spawnSync('npm', ['run', 'e2e:researchers:emulator:seed'], {
  cwd: supportDir,
  env,
  stdio: 'inherit',
  shell: process.platform === 'win32',
...
if (result.error) {
  console.error('\nUI seeding failed to start:', result.error.message);
  process.exit(1);
}

if (result.status !== 0) {
  console.error(`\nUI seeding failed with exit code ${result.status}`);
  process.exit(result.status || 1);
}

console.log('\n=== CYPRESS UI EMULATOR SEED COMPLETE ===');
