...
- **Tasks bootstrap:** `npm run emulator:seed:tasks` — copies only `registered == true` tasks and variants from a source project into the emulator, then derives `variantParamSpecs` from those variants.
...
`emulator:seed:functions` and `emulator:start:dashboard` use the same minimal bootstrap assumptions as the Cypress UI seed. The bootstrap creates only data the dashboard cannot create itself: system permissions, a super-admin user/claims, and task variants. Task variants are copied from a source project and filtered to `registered == true` for both tasks and variants; `variantParamSpecs` are then derived from those variants. Visible test fixtures are then created through the same callable Firebase Functions used by the dashboard (`setUidClaims`, `upsertOrg`, `createUsers`, `linkUsers`, and `upsertAdministration`).
