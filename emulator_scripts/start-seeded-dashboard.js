...
const firebaseBin = process.env.FIREBASE_BIN || path.resolve(rootDir, 'node_modules', '.bin', 'firebase');
const firebaseConfigPath = process.env.FIREBASE_CONFIG_PATH || path.resolve(rootDir, 'firebase.json');
const viteBin = process.env.VITE_BIN || path.resolve(appDir, 'node_modules', '.bin', 'vite');
...
function killOnPort(port) {
  spawnSync('bash', ['-lc', `if command -v lsof >/dev/null 2>&1; then lsof -ti:${port} | xargs -r kill -9; elif command -v fuser >/dev/null 2>&1; then fuser -k ${port}/tcp; fi`], {
    stdio: 'ignore',
  });
}
...
function startProcess(label, command, args, options) {
  ...
  child.on('exit', (code, signal) => {
    ...
    console.error(`${label} exited unexpectedly with code=${code} signal=${signal}. See ${options.logFile}`);
    cleanup(1);
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    function retry() {
      if (Date.now() - startedAt > timeoutMs) {
        reject(new Error(`${label} did not become ready at ${url} within ${timeoutMs}ms`));
        return;
      }
...
async function main() {
  ...
  startProcess(
    'Firebase emulators',
    firebaseBin,
    ['emulators:start', '--only', 'auth,firestore,functions', '--project', projectId, '--config', firebaseConfigPath],
    { cwd: rootDir, env: process.env, logFile: firebaseLog },
  );
  await waitForUrl(`http://127.0.0.1:${emulatorUiPort}`, 'Firebase Emulator UI');
  ...
  console.log('Seeding visible emulator dashboard data through Firebase Functions...');
