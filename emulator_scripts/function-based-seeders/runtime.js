...
function createFunctionsSeedRuntime({
  projectId,
  appName = "functions-seed-validation",
}) {
  ...
  const functionsOrigin =
    ...
    `http://127.0.0.1:${
      process.env.FIREBASE_FUNCTIONS_EMULATOR_PORT || "5002"
    }`;
  ...
  async function callFunction(name, data, idToken) {
    ...
    while (true) {
      try {
        ...
        const isFunctionsStartupRace =
          error.status === 404 || error.cause?.code === "ECONNREFUSED";
        if (!isFunctionsStartupRace || Date.now() - startedAt > 60_000)
          throw error;
  ...
  async function getVariantsByTaskIds(taskIds, idToken) {
    const entries = await Promise.all(
      taskIds.map(async (taskId) => [
        taskId,
        await getFirstVariant(taskId, idToken),
      ])
    );
    return Object.fromEntries(
      entries.filter(([, variant]) => Boolean(variant))
    );
  }
