...
function getTaskSourceProject(env = process.env) {
  return env.SEED_TASK_SOURCE_PROJECT || env.TASKS_SOURCE_PROJECT || 'hs-levante-admin-dev';
}

function getFunctionsSeedOptions(env = process.env) {
  const requestedProfile = env.SEED_PROFILE || 'dashboard';
  const profile = VALID_PROFILES.has(requestedProfile) ? requestedProfile : 'dashboard';
  ...
  return {
    ...
    studentCount,
    includeAdministrations: !['minimal', 'no-administrations', 'tasks-only'].includes(profile),
    includeOptionalAdministrationTemplates: parseBoolean(env.SEED_INCLUDE_OPTIONAL_ADMINISTRATION_TEMPLATES, false),
    validate: parseBoolean(env.SEED_VALIDATE, profile !== 'tasks-only' && profile !== 'minimal'),
  };
}
