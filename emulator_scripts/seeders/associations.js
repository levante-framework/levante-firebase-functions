...
async function linkUsersToGroups(adminApp, users, groups) {
  ...
  for (const user of participantUsers) {
    try {
      ...
      console.log(
        `    Linking ${user.email} to groups (${
          userClass === "newClass" ? "4th Grade" : "3rd Grade"
        })...`
      );
      ...
      const updateData = {
        ...
        "classes.current":
          admin.firestore.FieldValue.arrayUnion(assignedClassId),
      ...
      console.log(
        `      ✅ Linked ${user.email} (district, school, class ${assignedClassId}, group)`
      );
    } catch (error) {
      console.error(
        `      ❌ Failed to link ${user.email} to groups:`,
        error.message
      );
      throw error;
    }
