...
async function createSystemPermissions(adminApp) {
  const permissionsCore = await import(
  ...
  const buildFallbackPermissionMatrix = () => {
    const resourcePermissions = (resource) => {
      if (resource === 'groups') {
        return Object.fromEntries(ALL_GROUP_SUB_RESOURCES.map((subResource) => [subResource, ALL_ACTIONS]));
      }
      if (resource === 'admins') {
        return Object.fromEntries(ALL_ADMIN_SUB_RESOURCES.map((subResource) => [subResource, ALL_ACTIONS]));
      }
      return ALL_ACTIONS;
    };
    ...
    return Object.fromEntries(
      ALL_ROLES.map((role) => [
        role,
        Object.fromEntries(ALL_RESOURCES.map((resource) => [resource, resourcePermissions(resource)])),
      ]),
    );
  };
  const permissionMatrix = DEFAULT_PERMISSION_MATRIX || buildFallbackPermissionMatrix();
