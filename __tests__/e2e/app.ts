...
export function getClient() {
  const app = initClientApp({ projectId: PROJECT_ID, apiKey: 'fake-api-key' }, `e2e-${Date.now()}`);
...
export async function signInAs(client: ReturnType<typeof getClient>, uid: string, claims: Record<string, unknown>) {
...
export async function clearFirestore() {
  const res = await fetch(`http://127.0.0.1:8180/emulator/v1/projects/${PROJECT_ID}/databases/(default)/documents`, {
    method: 'DELETE',
  });
...
export async function clearAuth() {
  await fetch(`http://127.0.0.1:9199/emulator/v1/projects/${PROJECT_ID}/accounts`, { method: 'DELETE' });
}
...
export async function seedSystemPermissions() {
  const { DEFAULT_PERMISSION_MATRIX } = await import('@levante-framework/permissions-core');
