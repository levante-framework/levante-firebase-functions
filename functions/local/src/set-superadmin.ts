...
function ensureAnySuperAdminRole(roles: Role[]) {
...
const argv = yargs(hideBin(process.argv))
  .options({
    ...
    noUpdateUsersRoles: {
      description: "Do not update Firestore users/<uid>.roles",
      type: "boolean",
      default: false,
    },
  })
...
const nextCustomClaims = {
  ...
  admin: true,
  super_admin: true,
  useNewPermissions: Boolean((existingCustomClaims as any)?.useNewPermissions ?? true),
  rolesSet,
  roles: nextRoles,
  siteRoles,
  siteNames,
...
const updateUsersRoles = !Boolean(argv.noUpdateUsersRoles);
const willUpdateUsersRoles =
  updateUsersRoles && !rolesFromUsers.some((r) => r.siteId === "any" && r.role === "super_admin");
...
if (argv.apply) {
  ...
  if (willUpdateUsersRoles) await usersRef.set({ roles: nextRoles }, { merge: true });
}
